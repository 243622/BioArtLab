...
async def lifespan(app: FastAPI):
    # Schedule the task
    schedule.every().day.at("08:30").do(schedule_repopulate_daily_tasks)
...
async def get_all_tasks(db: db_dependency, limit: Union[int, None] = None, offset: Union[int, None] = None, done: Union[int, None] = None, task_type: Union[int, None] = None, priority: Union[int, None] = None, daily: Union[int, None] = None):
    query = db.query(models.Task)

    if done == 0 | 1:
        query = query.filter(models.Task.finished == done)
    if daily == 1:
        query = query.filter(models.Task.daily == 1)
    if task_type:
        query = query.filter(models.Task.taskType == task_type)
    query = query.order_by(desc(models.Task.createdDate))
...
@app.get('/tasksbydate', status_code=status.HTTP_200_OK)
async def get_tasks_by_date(
    date: str, 
    db: db_dependency, 
    limit: Union[int, None] = None,  # Optional limit
    offset: Union[int, None] = None,  # Optional offset
    priority: Union[int, None] = None,  # Optional priority filter
):
    query = db.query(models.Task)
    
    try:
        # Convert string date to datetime object (assumes 'yyyy-mm-dd' format)
        date_obj = datetime.strptime(date, "%Y-%m-%d")
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid date format. Use 'yyyy-mm-dd'.")
    
    # Define the range for the given date
    ...
    # Filter tasks created within the specific date range
    query = query.filter(models.Task.createdDate.between(start_of_day, end_of_day))
    
    # Optional: Filter by priority (if provided)
    if priority is not None:
        query = query.filter(models.Task.priority == priority)
    
    # Order by createdDate descending
    query = query.order_by(models.Task.createdDate.desc())
...
@app.get('/tasksbyrapportid/{rapport_id}', status_code=status.HTTP_200_OK)
async def get_task_by_id(db: db_dependency, rapport_id: int):
    return db.query(models.Task).filter(models.Task.rapportId == rapport_id).all()
...
@app.get('/rapports/{specific_date}', status_code=status.HTTP_200_OK)
async def get_rapport_by_date(
    db: db_dependency,
    specific_date: date):
    # Query to find rapports with the exact date
    query = db.query(models.Rapport).filter(models.Rapport.date == specific_date)
...
async def get_tasks_user(db: db_dependency, user_id: int, limit: Union[int, None] = None):
    query = db.query(models.UsersTasks).filter(models.UsersTasks.userId == user_id)
...
@app.get('/usertasks/{user_id}/fortoday', status_code=status.HTTP_200_OK)
async def get_tasks_user(db: db_dependency, user_id: int, limit: Union[int, None] = None):
    today = date.today()
    
    query = db.query(models.UsersTasks).filter(
        models.UsersTasks.userId == user_id,
        models.UsersTasks.createdDate == today
    )
...
async def create_user_task(db: db_dependency, usertask: schemes.UsersTasksBase):
    ...
    db_usertask.createdDate = date.today()
    db.add(db_usertask)
    db.commit()
...
def repopulate_daily_tasks(db: Session):
    tasks = db.query(models.Task).filter(models.Task.daily == 1).all()
    old_to_new_task_ids = {}
    for task in tasks:
        task_to_upload = schemes.TaskBase(
            title=task.title,
        ...
        new_task = create_task_sync(db, task_to_upload)
        old_to_new_task_ids[task.taskId] = new_task.taskId  # Map old task ID to new task ID
    return old_to_new_task_ids

def repopulate_daily_user_tasks(db: Session, old_to_new_task_ids: dict):
    user_tasks = db.query(models.UsersTasks).filter(models.UsersTasks.daily == 1).all()

    for user_task in user_tasks:
        # Use the new task ID from the mapping
        new_task_id = old_to_new_task_ids.get(user_task.taskId)
        if new_task_id is None:
            continue  # Skip if no matching new task ID exists

        task_to_upload = schemes.UsersTasksBase(
        ...
        create_user_task_sync(db, task_to_upload)

def create_task_sync(db: Session, task: schemes.TaskBase):
    db_task = models.Task(
        title=task.title,
        description=task.description,
        priority=task.priority,
...
def create_user_task_sync(db: Session, task: schemes.UsersTasksBase):
    db_task = models.UsersTasks(
        userId=task.userId,
        taskId=task.taskId,
        daily = 0,
        createdDate = task.createdDate
...
def schedule_repopulate_daily_tasks():
    db = next(get_db())  # Obtain a new session from the dependency
    print("Trying to repopulate")
    try:
        old_to_new_task_ids = repopulate_daily_tasks(db)
