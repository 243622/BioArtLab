...
class TaskServices {
  ...
  Future<List<Task>> getAll(
      [limit, offset, done, taskType, priority, daily]) async {
    try {
      final queryParams = {
      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 0; i < tasks.length; i++) {
          ...
          List<Future> futures = [];
          if (tasks[i].taskType != null)
            futures.add(taskTypeServices.getTaskTypeById(tasks[i].taskType!));
          if (tasks[i].stockId != null)
            futures.add(stockServices.getStockById(tasks[i].stockId!));
          if (tasks[i].animalTypeId != null)
            futures.add(
                animalTypeServices.getAnimalTypeById(tasks[i].animalTypeId!));
          ...
          if (tasks[i].stockId != null && resultIndex < results.length) {
            ...
            var responseToDecodeV = await foodTypeServices
                .getFoodTypeById(responseToStock.foodTypeId);
  ...
  Future<List<Task>> getAllByDate(DateTime date,
      [limit, offset, done, taskType, priority]) async {
    try {
      final queryParams = {
        if (limit != null) 'limit': limit.toString(),
        if (offset != null) 'offset': offset.toString(),
      ...
      const url = '$_baseApi/tasksbydate';
      final uri = Uri.parse(url).replace(queryParameters: queryParams);
      final response = await http.get(uri);
  ...
  Future<List<Task>> getAllByDaily(
      [limit, offset, done, taskType, priority]) async {
    try {
      final queryParams = {
        if (limit != null) 'limit': limit.toString(),
      ...
      const url = '$_baseApi/tasksbydaily';
      final uri = Uri.parse(url).replace(queryParameters: queryParams);
      final response = await http.get(uri);
      if (response.statusCode == 200) {
        final json = jsonDecode(response.body) as List;
  ...
  Future<Task?> getTaskById(int taskId) async {
    try {
      final url = '$_baseApi/task/$taskId';
      ...
      await Future.wait([
        ...
        if (task.stockId != null)
          stockServices
              .getStockById(task.stockId!)
              .then((responseToStock) async {
            if (responseToStock?.foodTypeId != null) {
              final foodTypeResponse = await foodTypeServices
                  .getFoodTypeById(responseToStock!.foodTypeId);
        ...
        if (task.animalTypeId != null)
          animalTypeServices
              .getAnimalTypeById(task.animalTypeId!)
              .then((responseToAnimalType) {
  ...
  Future getTasksByRapportId(int rapportId) async {
    try {
      final url = '$_baseApi/tasksbyrapportid/$rapportId';
      final uri = Uri.parse(url);
      final response = await http.get(uri);
      final json = jsonDecode(response.body) as List;
      final tasks = json.map((e) {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        Task task = Task(
          ...
          doneDate: taskToDecode['doneDate'] != null
              ? DateTime.parse(taskToDecode['doneDate'])
              : null,
          deadline: taskToDecode['deadline'] != null
              ? DateTime.parse(taskToDecode['deadline'])
              : null,
          createdDate: taskToDecode["createdDate"] != null
              ? DateTime.parse(taskToDecode["createdDate"])
              : null,
  ...
  Future deleteTask(int taskId) async {
    try {
      final url = '$_baseApi/tasks/$taskId';
      final uri = Uri.parse(url);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final task = Task(
            ...
            doneDate: jsonTask['doneDate'] != null
                ? DateTime.parse(jsonTask['doneDate'])
                : null,
            deadline: jsonTask['deadline'] != null
                ? DateTime.parse(jsonTask['deadline'])
                : null,
            createdDate: jsonTask["createdDate"] != null
                ? DateTime.parse(jsonTask["createdDate"])
                : null,
        ...
        if (task.taskType != null) {
          var responseToDecode =
              await taskTypeServices.getTaskTypeById(task.taskType!);
        ...
        if (task.animalTypeId != null) {
          var responseToAnimalType =
              await animalTypeServices.getAnimalTypeById(task.animalTypeId!);
          task.animalTypeInString = responseToAnimalType.name;
        }
